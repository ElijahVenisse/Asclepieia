        ...
        <?php
                ...
                if (is_array($herbData['herb'])) {
                    ...
                    if (!empty($plantsWithClickCount)) {
                        ...
                        foreach ($topClickedPlants as $plant) {
                            ...
                            echo '<h5 class="card-title" style="display: flex; justify-content: space-between;">' . $plant['name'] . '<span><i class="fa fa-eye"></i>  ' . $plant['clickCount'] . '</span></h5>';
                            echo '</a>';
                        }
                        echo '</div>';
                        echo '</div>';
            ...
            if (!empty($herbData['herb'])) {
                $plantsWithCommentCount = array_filter($herbData['herb'], function ($plant) {
                    return isset($plant['comments']);
                });
        
                usort($plantsWithCommentCount, function ($a, $b) {
                    $aCount = count($a['comments']);
                    $bCount = count($b['comments']);
                ...
                if (!empty($plantsWithCommentCount)) {
                    ...
                    foreach ($topCommentedPlants as $plant) {
                        ...
                        if (isset($plant['comments'])) echo '<p>'. count($plant['comments']) . ' comments</p>';
                        else echo '<p> 0 comments </p>';
                        echo '</a>';
                    }
                    echo '</div>';
                    echo '</div>';
